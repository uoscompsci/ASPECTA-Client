...
class client:
	...
	def splitSide(self, circles, side, surface):
		...
		if(count<17): #Restrict to a maximum of 15 waypoints per side
			...
			for x in reversed(range(0,len(insert))):
				ele = self.sender.newCircle(1, insert[x][0], int(insert[x][1]), 7, "pix", (1, 0, 0, 0), 1, (0, 1, 0, 1), 10)
				...
				if event.type == pygame.MOUSEBUTTONDOWN:
					...
					#Runs if the right mouse button is pressed
							...
							if(self.controlCur==self.mainCur):
								...
								for z in range(0,len(self.topCircles)):
									...
									if(hit==True):
										...
										self.symbolicDrag[1] = self.sender.newCircle(1, loc[0], loc[1], 10, "pix", (1, 0, 0, 0), 1, (1, 0, 0, 1), 10)
					elif event.button==1:
						...
						if(self.controlCur==self.mainCur):
							...
							if(get_point==True):
								loc = self.sender.getCursorPosition(self.controlCur)
								ele = self.sender.newCircle(1, loc[0], loc[1], 10, "pix", (1, 0, 0, 0), 1, (1, 1, 0, 1), 10)
								self.hideable.append(ele)
								return (loc, ele)
					...
					if(event.button==1):
						...
						if(self.controlCur==self.mainCur):
							...
							if len(self.stretching)>0:
								...
								self.sender.relocateRectangle(self.stretchRects[self.stretching[0][0]], newWidth/2-75, newHeight/2+75, "pix", self.surfCanvases[self.warpedSurf[self.stretching[0][0]]-1])
								self.sender.relocateCircle(self.stretchCircs[self.stretching[0][0]][0], newWidth/2, newHeight/2+75, "pix", self.surfCanvases[self.stretching[0][0]])
								self.sender.relocateCircle(self.stretchCircs[self.stretching[0][0]][1], newWidth/2, newHeight/2-75, "pix", self.surfCanvases[self.stretching[0][0]])
								self.sender.relocateCircle(self.stretchCircs[self.stretching[0][0]][2], newWidth/2-75, newHeight/2, "pix", self.surfCanvases[self.stretching[0][0]])
								self.sender.relocateCircle(self.stretchCircs[self.stretching[0][0]][3], newWidth/2+75, newHeight/2, "pix", self.surfCanvases[self.stretching[0][0]])
						...
						for x in self.stretching:
							...
							if(x[1]<2):
								if(x[1]==0 and loc[1]>hei/2 or x[1]==1 and loc[1]<hei/2):
									self.sender.relocateCircle(self.stretchCircs[x[0]][x[1]], float(origpos[0]), float(loc[1]), "pix", self.surfCanvases[x[0]])
									hdifference = loc[1] - origpos[1]
									if(x[1]==0):
										origpos2 = self.sender.getCirclePosition(self.stretchCircs[x[0]][1])
										self.sender.relocateCircle(self.stretchCircs[x[0]][1], float(origpos[0]), float(origpos2[1])-hdifference, "pix", self.surfCanvases[x[0]])
									else:
										origpos2 = self.sender.getCirclePosition(self.stretchCircs[x[0]][0])
										self.sender.relocateCircle(self.stretchCircs[x[0]][0], float(origpos[0]), float(origpos2[1])-hdifference, "pix", self.surfCanvases[x[0]])
							else:
								if(x[1]==2 and loc[0]<wid/2 or x[1]==3 and loc[0]>wid/2):
									self.sender.relocateCircle(self.stretchCircs[x[0]][x[1]], float(loc[0]), float(origpos[1]), "pix", self.surfCanvases[x[0]])
									vdifference = loc[0] - origpos[0]
									if(x[1]==2):
										origpos2 = self.sender.getCirclePosition(self.stretchCircs[x[0]][3])
										self.sender.relocateCircle(self.stretchCircs[x[0]][3], float(origpos2[0])-vdifference, float(origpos[1]), "pix", self.surfCanvases[x[0]])
									else:
										origpos2 = self.sender.getCirclePosition(self.stretchCircs[x[0]][2])
										self.sender.relocateCircle(self.stretchCircs[x[0]][2], float(origpos2[0])-vdifference, float(origpos[1]), "pix", self.surfCanvases[x[0]])
							self.updateRectangle(x[0])
							
	def updateRectangle(self, surfaceRectNo):
		top = self.sender.getCirclePosition(self.stretchCircs[surfaceRectNo][0])[1]
		bottom = self.sender.getCirclePosition(self.stretchCircs[surfaceRectNo][1])[1]
		left = self.sender.getCirclePosition(self.stretchCircs[surfaceRectNo][2])[0]
		...
		self.sender.relocateRectangle(self.stretchRects[surfaceRectNo], left, top, "pix", self.surfCanvases[surfaceRectNo])
		...
		for x in range(0,len(layout)):
			...
			for z in range(0,len(layout[x][y])):
				...
				if ((z==0) or (z==(len(layout[x][y])-1))):
					ele = self.sender.newCircle(1, layout[x][y][z][0], layout[x][y][z][1], 10, "pix", (1, 0, 0, 0), 1, (1, 1, 0, 1), 10)
				else:
					ele = self.sender.newCircle(1, layout[x][y][z][0], layout[x][y][z][1], 7, "pix", (1, 0, 0, 0), 1, (0, 1, 0, 1), 10)
			...
			center = self.lineIntersection(tlcoor[0], tlcoor[1], brcoor[0], brcoor[1], trcoor[0], trcoor[1], blcoor[0], blcoor[1])
			self.centerPoints[self.surfaceCounter] = self.sender.newCircle(1, center[0], center[1], 10, "pix", (0,0,0,0), 1, (1,0,1,1), 10)
	...
	def tkinthread(self):
		...
		self.label.pack(side=LEFT)
		layoutScroll = Scrollbar(self.frame6, orient=VERTICAL)
		self.loadList = Listbox(self.frame6, width=41, yscrollcommand=layoutScroll.set)
		...
		self.loadLayout()
		self.saveBut = Button(self.frame7, text="Load Layout", command=self.loadLayout, width=18)
		self.saveBut.pack(side=LEFT)
		self.saveBut = Button(self.frame7, text="Refresh List", command=self.refreshLayouts, width=18)
		self.saveBut.pack(side=LEFT)
		self.saveBut = Button(self.frame8, text="Delete Layout", command=self.deleteLayout, width=18)
		self.saveBut.pack(side=LEFT)
	...
	def initGUI(self):
		...
		self.stretchRects[0] = self.sender.newRectangle(self.canvas, 512/2-75, 512/2+75, 150, 150, "pix", (0,0,0,0), 1, (0,0,1,1))
		self.centSurfCirc[0] = self.sender.newCircle(self.canvas, 512/2, 512/2, 25, "pix", (0,0,0,0), 0, (1,1,0,1), 10)
		self.sender.hideElement(self.centSurfCirc[0])
		self.sender.hideElement(self.stretchRects[0])
		circs = {}
		circs[0] = self.sender.newCircle(self.canvas, 512/2, 512/2+75, 15, "pix", (0,0,0,0), 1, (0,1,0,1), 10)
		self.sender.hideElement(circs[0])
		circs[1] = self.sender.newCircle(self.canvas, 512/2, 512/2-75, 15, "pix", (0,0,0,0), 1, (0,1,0,1), 10)
		self.sender.hideElement(circs[1])
		circs[2] = self.sender.newCircle(self.canvas, 512/2-75, 512/2, 15, "pix", (0,0,0,0), 1, (0,1,0,1), 10)
		self.sender.hideElement(circs[2])
		circs[3] = self.sender.newCircle(self.canvas, 512/2+75, 512/2, 15, "pix", (0,0,0,0), 1, (0,1,0,1), 10)
		self.sender.hideElement(circs[3])
		self.topmtrrect[0] = self.sender.newRectangle(self.canvas, 512/2 - 80, 512, 160, 75, "pix", (0,0,0,0), 0, (1,1,1,1))
		topCount1 = self.sender.newText(self.canvas, "0", 512/2-75, 460, "pix", 37, "Arial", (0,0,0,1))
		topCount2 = self.sender.newText(self.canvas, "0", 512/2-50, 460, "pix", 37, "Arial", (0,0,0,1))
		topCount3 = self.sender.newText(self.canvas, "0", 512/2-25, 460, "pix", 37, "Arial", (0,0,0,1))
		topCount4 = self.sender.newText(self.canvas, "0", 512/2, 460, "pix", 37, "Arial", (0,0,0,1))
		self.topCounter[0] = (topCount1, topCount2, topCount3, topCount4)
		self.topcm[0] = self.sender.newText(self.canvas, "cm", 512/2+25, 460, "pix", 35, "Arial", (0,0,0,1))
		self.rightmtrrect[0] = self.sender.newRectangle(self.canvas, 512-180, 512/2+30, 180, 60, "pix", (0,0,0,0), 0, (1,1,1,1))
		rightCount1 = self.sender.newText(self.canvas, "0", 512-175, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		rightCount2 = self.sender.newText(self.canvas, "0", 512-150, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		rightCount3 = self.sender.newText(self.canvas, "0", 512-125, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		rightCount4 = self.sender.newText(self.canvas, "0", 512-100, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		self.rightCounter[0] = (rightCount1, rightCount2, rightCount3, rightCount4)
		self.rightcm[0] = self.sender.newText(self.canvas, "cm", 512-65, 512/2-13, "pix", 35, "Arial", (0,0,0,1))
		...
		self.stretchRects[1] = self.sender.newRectangle(self.canvas, 512/2-75, 512/2+75, 150, 150, "pix", (0,0,0,0), 1, (0,0,1,1))
		self.centSurfCirc[1] = self.sender.newCircle(self.canvas, 512/2, 512/2, 25, "pix", (0,0,0,0), 0, (1,1,0,1), 10)
		...
		circs[3] = self.sender.newCircle(self.canvas, 512/2+75, 512/2, 15, "pix", (0,0,0,0), 1, (0,1,0,1), 10)
		self.sender.hideElement(circs[3])
		self.topmtrrect[1] = self.sender.newRectangle(self.canvas, 512/2 - 80, 512, 160, 75, "pix", (0,0,0,0), 0, (1,1,1,1))
		topCount1 = self.sender.newText(self.canvas, "0", 512/2-75, 460, "pix", 37, "Arial", (0,0,0,1))
		topCount2 = self.sender.newText(self.canvas, "0", 512/2-50, 460, "pix", 37, "Arial", (0,0,0,1))
		topCount3 = self.sender.newText(self.canvas, "0", 512/2-25, 460, "pix", 37, "Arial", (0,0,0,1))
		topCount4 = self.sender.newText(self.canvas, "0", 512/2, 460, "pix", 37, "Arial", (0,0,0,1))
		self.topCounter[1] = (topCount1, topCount2, topCount3, topCount4)
		self.topcm[1] = self.sender.newText(self.canvas, "cm", 512/2+25, 460, "pix", 35, "Arial", (0,0,0,1))
		self.rightmtrrect[1] = self.sender.newRectangle(self.canvas, 512-180, 512/2+30, 180, 60, "pix", (0,0,0,0), 0, (1,1,1,1))
		rightCount1 = self.sender.newText(self.canvas, "0", 512-175, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		rightCount2 = self.sender.newText(self.canvas, "0", 512-150, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		rightCount3 = self.sender.newText(self.canvas, "0", 512-125, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		rightCount4 = self.sender.newText(self.canvas, "0", 512-100, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		self.rightCounter[1] = (rightCount1, rightCount2, rightCount3, rightCount4)
		self.rightcm[1] = self.sender.newText(self.canvas, "cm", 512-65, 512/2-13, "pix", 35, "Arial", (0,0,0,1))
		...
		self.stretchRects[2] = self.sender.newRectangle(self.canvas, 512/2-75, 512/2+75, 150, 150, "pix", (0,0,0,0), 1, (0,0,1,1))
		self.centSurfCirc[2] = self.sender.newCircle(self.canvas, 512/2, 512/2, 25, "pix", (0,0,0,0), 0, (1,1,0,1), 10)
		...
		circs[3] = self.sender.newCircle(self.canvas, 512/2+75, 512/2, 15, "pix", (0,0,0,0), 1, (0,1,0,1), 10)
		self.sender.hideElement(circs[3])
		self.topmtrrect[2] = self.sender.newRectangle(self.canvas, 512/2 - 80, 512, 160, 75, "pix", (0,0,0,0), 0, (1,1,1,1))
		topCount1 = self.sender.newText(self.canvas, "0", 512/2-75, 460, "pix", 37, "Arial", (0,0,0,1))
		topCount2 = self.sender.newText(self.canvas, "0", 512/2-50, 460, "pix", 37, "Arial", (0,0,0,1))
		topCount3 = self.sender.newText(self.canvas, "0", 512/2-25, 460, "pix", 37, "Arial", (0,0,0,1))
		topCount4 = self.sender.newText(self.canvas, "0", 512/2, 460, "pix", 37, "Arial", (0,0,0,1))
		self.topCounter[2] = (topCount1, topCount2, topCount3, topCount4)
		self.topcm[2] = self.sender.newText(self.canvas, "cm", 512/2+25, 460, "pix", 35, "Arial", (0,0,0,1))
		self.rightmtrrect[2] = self.sender.newRectangle(self.canvas, 512-180, 512/2+30, 180, 60, "pix", (0,0,0,0), 0, (1,1,1,1))
		rightCount1 = self.sender.newText(self.canvas, "0", 512-175, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		rightCount2 = self.sender.newText(self.canvas, "0", 512-150, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		rightCount3 = self.sender.newText(self.canvas, "0", 512-125, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		rightCount4 = self.sender.newText(self.canvas, "0", 512-100, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		self.rightCounter[2] = (rightCount1, rightCount2, rightCount3, rightCount4)
		self.rightcm[2] = self.sender.newText(self.canvas, "cm", 512-65, 512/2-13, "pix", 35, "Arial", (0,0,0,1))
		...
		self.stretchRects[3] = self.sender.newRectangle(self.canvas, 512/2-75, 512/2+75, 150, 150, "pix", (0,0,0,0), 1, (0,0,1,1))
		self.centSurfCirc[3] = self.sender.newCircle(self.canvas, 512/2, 512/2, 25, "pix", (0,0,0,0), 0, (1,1,0,1), 10)
		...
		circs[3] = self.sender.newCircle(self.canvas, 512/2+75, 512/2, 15, "pix", (0,0,0,0), 1, (0,1,0,1), 10)
		self.sender.hideElement(circs[3])
		self.topmtrrect[3] = self.sender.newRectangle(self.canvas, 512/2 - 80, 512, 160, 75, "pix", (0,0,0,0), 0, (1,1,1,1))
		topCount1 = self.sender.newText(self.canvas, "0", 512/2-75, 460, "pix", 37, "Arial", (0,0,0,1))
		topCount2 = self.sender.newText(self.canvas, "0", 512/2-50, 460, "pix", 37, "Arial", (0,0,0,1))
		topCount3 = self.sender.newText(self.canvas, "0", 512/2-25, 460, "pix", 37, "Arial", (0,0,0,1))
		topCount4 = self.sender.newText(self.canvas, "0", 512/2, 460, "pix", 37, "Arial", (0,0,0,1))
		self.topCounter[3] = (topCount1, topCount2, topCount3, topCount4)
		self.topcm[3] = self.sender.newText(self.canvas, "cm", 512/2+25, 460, "pix", 35, "Arial", (0,0,0,1))
		self.rightmtrrect[3] = self.sender.newRectangle(self.canvas, 512-180, 512/2+30, 180, 60, "pix", (0,0,0,0), 0, (1,1,1,1))
		rightCount1 = self.sender.newText(self.canvas, "0", 512-175, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		rightCount2 = self.sender.newText(self.canvas, "0", 512-150, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		rightCount3 = self.sender.newText(self.canvas, "0", 512-125, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		rightCount4 = self.sender.newText(self.canvas, "0", 512-100, 512/2-13, "pix", 37, "Arial", (0,0,0,1))
		self.rightCounter[3] = (rightCount1, rightCount2, rightCount3, rightCount4)
		self.rightcm[3] = self.sender.newText(self.canvas, "cm", 512-65, 512/2-13, "pix", 35, "Arial", (0,0,0,1))
